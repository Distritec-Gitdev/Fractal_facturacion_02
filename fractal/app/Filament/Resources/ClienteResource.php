        ...
        return $form
            ...
            Forms\Components\TextInput::make('nombre_asesor')
                ...
                Section::make('Estado del Crédito')
                        ...
                        ->label('Estado')
                        ->options(fn () => \App\Models\ZEstadocredito::pluck('Estado_Credito', 'ID_Estado_cr')->toArray())
                        ->searchable()
                        ->preload()
                        // Puedes dejar live si quieres reactividad visual, pero ya no guardes aquí nada:
                        ->live()
                        // IMPORTANT: ahora sí que se guarde al submit
                        ->dehydrated(true)
                        ->disabled(fn () => auth()->user()?->hasRole('administrativo')
                            || auth()->user()?->hasAnyRole(['asesor comercial', 'asesor_agente'])
                        ),
...
public static function table(Table $table): Table
{
    ...
    return $table
        ...
        ->columns([
         ...
         //      })
         //      ->disabled(function () {
         //          // Bloqueado siempre para el rol "administrativo"
         ...
         //          // Igual que en Nombre Auxiliar: suprimir loader para este select
         //          $a['x-data'] = '{}';
         //          $a['x-on:change.capture'] = "
         //              if (window.AppLoader) {
         ...
         //          // Usamos solo la relación eager loaded (sin ZEstadocredito::find)
